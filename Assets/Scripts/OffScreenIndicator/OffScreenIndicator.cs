...
public class OffScreenIndicator : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        // 主动注册已存在的 Target（修复 Target 先创建时不触发事件的问题）
    ...
    void DrawIndicators()
    {
        foreach(Target target in targets)
        {
            ...
            if(indicator)
            {
                ...
                indicator.transform.position = Vector3.Lerp(indicator.transform.position, screenPosition, 10f * Time.deltaTime);         //Sets the position of the indicator on the screen.
    ...
    {
        foreach (var target in UnityEngine.Object.FindObjectsByType<Target>(FindObjectsSortMode.None))
        {
            HandleTargetStateChanged(target, true);
        }
    }
}
